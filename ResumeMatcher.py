...
def getWordCount(lst, nPrint):
    ...
    for word in lst.lower().split():
        ...
        if finalStr not in stwords and finalStr not in stopwords2 and finalStr != '' and len(finalStr) > 3:
...
def getResumeMatchScore(job, resume, positionText, resumeWords, positionWords):
    ...
    keyWordSimilarity = sum(maxSims) / len(maxSims) * 1000
    ...
    sentSimilarity = sum(maxSentSims[0:10]) / len(maxSentSims[0:10]) * 1000
    overallSimilarity += sentSimilarity 


    print('Sentence Similarity:', sentSimilarity)

    print('\nFinal Score:', overallSimilarity)
    print()
...
def findResumeMatches():
    ...
    Calculate the corresponding similarity score 
    '''

    for row in csvRows[0 : 2]:
        ...
        for job in row[0 : 2]:
            ...
            positionWords = getWordCount(p, 15)
            
            keywordSimi, sentSimi, overallSimi = getResumeMatchScore(job, resume, positionText, resumeWords, positionWords)

            allData.append([job[0], job[1], job[2], job[3], job[4], job[5], keywordSimi, sentSimi, overallSimi])


    # Column Names in the excel file  
    fields = 'Title, Company, Rating, Location, Summary, Job Link, KeyWord Similarity, Sentence Similarity, Overall Similarity\n' 
    ...
    for job in allData:
        MyFile.write(clean(job[0]) + ',' + clean(job[1]) + ',' + clean(job[2]) + ',' + clean(job[3]) + ',' + clean(job[4]) + ',' + clean(job[5]) + ',' + str(job[6]) + ',' + str(job[7]) + ',' + str(job[8]))
        MyFile.write('\n')
